...
import { QrCode, Loader2, CheckCircle2, Signal } from "lucide-react";
...
export function ChatbotContent({ userId, permission }: ChatbotContentProps) {
  ...
  // ✅ Pega a configuração do chatbot
  const { data: config, isLoading } = useQuery({
    queryKey: ["chatbot-config", userId],
    queryFn: async () => {
      const response = await fetch(`/api/chatbot/get-config?userId=${userId}`);
      if (!response.ok) throw new Error("Falha ao buscar configurações do chatbot.");
      return response.json();
    },
    // ⚠️ Desativa o refetch automático (pode reativar depois se quiser)
  ...
  // ✅ Atualiza os valores quando o config mudar
  useEffect(() => {
    if (config) {
      form.reset(config);
    }
  ...
  if (isLoading) return <p>Carregando configurações do chatbot...</p>;
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      {/* CARD 1 - CONFIGURAÇÕES DO CHATBOT */}
      <Card className="p-6">
        <CardHeader>
          <CardTitle className="text-2xl font-bold">Configurações do Chatbot</CardTitle>
          <CardDescription>Personalize o seu assistente virtual para atender os clientes via WhatsApp.</CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                    <div className="space-y-0.5">
                      ...
                      <FormDescription>Ative ou desative seu assistente virtual.</FormDescription>
                    </div>
                    <FormControl>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea
                        placeholder="Ex: Gentil, amigável, educado e profissional"
                        className="max-h-52"
                        {...field}
                      />
                    </FormControl>
        ...
        <CardContent className="flex flex-col items-center justify-center space-y-4">
          {!isConnected && !showQrCode && (
            <Button onClick={handleGenerateQrCode} disabled={!form.watch("enabled") || isGenerating}>
              {isGenerating ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <QrCode className="w-4 h-4 mr-2" />}
          ...
          {showQrCode && (
            <div className="border p-4 rounded-md flex flex-col items-center justify-center min-h-[300px] w-[300px] text-center">
              {statusMessage && <p className="text-sm text-gray-600 font-semibold">{statusMessage}</p>}
          ...
          {isConnected && (
            <div className="border border-green-500 bg-green-100 p-4 rounded-md text-center animate-fade-in">
